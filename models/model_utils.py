...
def conv(batchNorm, cin, cout, k=3, stride=1, pad=-1):
    ...
    if batchNorm:
        ...
        return nn.Sequential(
                ...
                nn.BatchNorm2d(cout, affine=False),
                )
    else:
        return nn.Sequential(
                nn.Conv2d(cin, cout, kernel_size=k, stride=stride, padding=pad, bias=True),
...
# modified
# def conv(batchNorm, cin, cout, k=3, stride=1, pad=-1, affine=True):
#     pad = (k - 1) // 2 if pad < 0 else pad
#     print('Conv pad = %d' % (pad))
#     if batchNorm:
#         print('=> convolutional layer with bachnorm')
#         return nn.Sequential(
#                 nn.Conv2d(cin, cout, kernel_size=k, stride=stride, padding=pad),
#                 nn.LeakyReLU(0.1, inplace=True),
...
def conv3d(batchNorm, cin, cout, k=3, stride=1, pad=-1, atten=None):
    ...
    layers=[nn.Conv3d(cin, cout, kernel_size=k, stride=stride, padding=pad, bias=True)]
...
# modifed
# def conv3d(batchNorm, cin, cout, k=3, stride=1, pad=-1, atten=None, affine=True):
#
#     layers=[nn.Conv3d(cin, cout, kernel_size=k, stride=stride, padding=pad)]
#     layers.append(nn.LeakyReLU(0.1, inplace=True))
#     if atten is not None:
#         layers.append(Attention_layer(cout))
#     if batchNorm:
#         layers.append(nn.BatchNorm3d(cout, affine=affine))
#
#     return nn.Sequential(*layers)
...
def deconv3d(cin, cout):
    return nn.Sequential(
            nn.ConvTranspose3d(cin, cout, kernel_size=[1,4, 4], stride=[1,2, 2], padding=[0,1, 1], bias=True),
            nn.LeakyReLU(0.1, inplace=True)
            )
