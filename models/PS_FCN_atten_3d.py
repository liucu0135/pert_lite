...
class FeatExtractor(nn.Module):
    def __init__(self, batchNorm=False, c_in=3, other={}):
        ...
        # less atten
        self.conv1 = model_utils.conv3d(batchNorm, 6, 64,  k=[1,3,3], stride=1, pad=[0,1,1])
        self.at1=Attention_layer(64)
        self.bn1=nn.BatchNorm3d(64, affine=False)
        self.pool1 = torch.nn.MaxPool3d(kernel_size=[2,1,1], stride=[2, 1, 1], padding=[0, 0, 0])
        self.conv2 = model_utils.conv3d(batchNorm, 64,  128, k=[1,3,3], stride=1, pad=[0,1,1])
        self.conv21 = model_utils.conv3d(batchNorm, 128,  128, k=[1,3,3], stride=1, pad=[0,1,1])
        self.at2=Attention_layer(128)
        self.bn2=nn.BatchNorm3d(128, affine=False)
        self.pool2 = torch.nn.MaxPool3d(kernel_size=[2,1,1], stride=[2, 1, 1], padding=[0, 0, 0])
        self.conv3 = model_utils.conv3d(batchNorm, 128,   256, k=[1,3,3], stride=[1,2,2], pad=[0,1,1])
        self.conv4 = model_utils.conv3d(batchNorm, 256,  256, k=[1,3,3], stride=1, pad=[0,1,1])
        self.conv41 = model_utils.conv3d(batchNorm, 256,  256, k=[1,3,3], stride=[1,1,1], pad=[0,1,1])
        self.at3=Attention_layer(256)
        self.bn3=nn.BatchNorm3d(256, affine=False)
        self.pool3 = torch.nn.MaxPool3d(kernel_size=[2,1,1], stride=[2, 1, 1], padding=[0, 0, 0])
