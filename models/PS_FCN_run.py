...
class Regressor(nn.Module):
    def __init__(self, batchNorm=False, other={},c_out=3):
        ...
        self.est_normal= self._make_output(64, c_out, k=3, stride=1, pad=1)
        self.other   = other
        self.c_out=c_out
...
class PS_FCN(nn.Module):
    def __init__(self, fuse_type='max', batchNorm=False, c_in=3, other={},c_out=3):
        super(PS_FCN, self).__init__()
        self.extractor = FeatExtractor(batchNorm, c_in, other)
        self.regressor = Regressor(batchNorm, other, c_out=c_out)
        self.c_in      = c_in
