...
def read_light_dir(list_path,ignore_head=False, sort=True):
    with open(list_path) as f:
        lists = f.read().splitlines()
    fromstring=lambda n: [np.fromstring(nn.split(':')[1], dtype=float, sep=' ') for nn in n]
    light_dir=fromstring(lists)
    return np.stack(light_dir)



def read_light_intens(list_path,ignore_head=False, sort=True):
    with open(list_path) as f:
        lists = f.read().splitlines()
    fromstring=lambda n: [np.fromstring(nn, dtype=float, sep=' ')[1:] for nn in n]
