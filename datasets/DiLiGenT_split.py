...
class DiLiGenT_main(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        index=16*19+19
        row_idx=index%(16*19)%16
        col_idx=(index-row_idx)%(16*19)//16
        img_index=(index-row_idx-col_idx*16)//19//16
        index=img_index
        ...
        normal = normal['Normal_gt'][row_idx: row_idx + 32, col_idx * 16: col_idx * 16 + 32]

        imgs = []
        for idx, img_name in enumerate(img_list):
            img = imread(img_name).astype(np.float32) / 255.0
            img = img[row_idx:row_idx+32,col_idx*16:col_idx*16+32]# splitting image
        ...
        mask=mask[row_idx: row_idx + 32, col_idx * 16: col_idx * 16 + 32]
        # down = 4
